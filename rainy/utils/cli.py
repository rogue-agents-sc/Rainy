...
@rainy_cli.command()
@click.argument("savedir", required=True, type=str)
@click.option("--model", type=str, default=SAVE_FILE_DEFAULT)
@click.option("--num-episodes", type=int, default=10)
@click.option("--reward-thresh", type=int, default=None)
@click.pass_context
def episodes(ctx: dict, savedir: str, model: str, num_episodes: int, reward_thresh:int) -> None:
    c = ctx.obj["config"]
    ag = ctx.obj["make_agent"](c)
    gen_episodes(
        ag,
        savedir,
        load_file_path=model,
...
def run_cli(
    ...
    obj = {"config": config, "make_agent": agent_gen, "script_path": script_path}
    rainy_cli(obj=obj)
